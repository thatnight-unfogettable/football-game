...
export function limited(key: string, limit = 20, windowMs = 5000): boolean {
  const now = Date.now();
...
export function broadcast(room: any) {
...
export function setDeadline(room: any, seconds: number) {
...
export function applyEngineSide(_game: any, side: string) {
  return playerSideToEngine(side);
}

export function chooseBest(game: any, side: string, action: string) {
  const engine = applyEngineSide(game, side);
  const ids = available(game);
  if (action === 'pick') {
    return [...ids].sort((a, b) => {
      const pa = game.players.find((p: any) => p.id === a);
      const pb = game.players.find((p: any) => p.id === b);
...
export function timeoutAction(room: any) {
  ...
  if (g.phase === 'LINEUP') {
    for (const side of ['A', 'B'] as const) room.players[side].lineupReady = true;
    finishLineupIfReady(room);
  }
}

export function handleOrder(room: any, side: string, choice: string, auto = false) {
...
export function handleAction(room: any, side: string, type: string, id: string, auto = false) {
  const g = room.game;
  const engineSide = playerSideToEngine(side);
  if (type === 'BAN') {
...
export function advance(room: any, side: string) {
...
export function lineupSwap(room: any, side: string, a: string, b: string) {
...
export function finishLineupIfReady(room: any) {
...
export function finishByForfeit(room: any, loser: string, reason: string) {
...
export function createRoom(ws: WebSocket, nickname: string) {
  const code = roomCode();
  const side = 'A';
  const session = token();
  const room: any = {
...
export function joinRoom(ws: WebSocket, code: string, nickname: string) {
...
export function reconnect(ws: WebSocket, code: string, session: string) {
  const room = rooms.get(code);
...
export function onMessage(ws: WebSocket, raw: string, ip: string) {
  ...
  if (limited(`${ip}:${msg.type}`)) return send(ws, 'ERROR', { message: '操作过于频繁' });
  const p = msg.payload || {};
  if (msg.type === 'CREATE') {
  ...
  const room = rooms.get((ws as any).room);
  const side = (ws as any).side;
...
export function startMatch(room: any) {
...
export function preparePlayers() {
  return PLAYER_DATA.map((p: any) => ({ ...p, nameZh: CN_NAMES[p.englishName] || p.name, alternativePositions: p.alternativePositions || [] }));
}

// Import PLAYER_DATA - in real deployment, this would be bundled or fetched
// For Cloudflare Workers, we need to embed or fetch player data
declare const PLAYER_DATA: any[];
