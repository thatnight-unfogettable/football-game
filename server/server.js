...
function joinRoom(ws,code,nickname){const room=rooms.get(code);if(!room)return send(ws,'ERROR',{message:'房间不存在'});if(room.players.B)return send(ws,'ERROR',{message:'房间已满'});if(room.players.A.nickname===nickname)return send(ws,'ERROR',{message:'昵称已被使用'});const session=token();room.players.B={nickname,ready:true,ws,session,continueReady:false,lineupReady:false};ws.room=code;ws.side='B';send(ws,'SESSION',{code,side:'B',token:session});broadcast(room);console.log(`[room] joined ${code}`);}
