...
import { optimizeLayout, simulateSeries, swapSlots } from '../src/game.js';
...
function startMatch(room){const rounds=buildRounds();room.game={version:2,seed:Date.now(),phase:'ORDER',round:0,rounds,candidates:[],roundType:rounds[0].type,roundHint:rounds[0].hint,firstPicker:null,firstBan:null,banTurn:0,banCount:0,prePicks:[],postPicks:[],pickOwners:{},roundBans:[],bans:[],picks:{A:['shared_courtois'],B:['shared_courtois']},layouts:null,matches:[],winner:null,logs:[]};room.status='playing';room.choiceOwner=Math.random()<.5?'A':'B';room.history=[];setDeadline(room,15);console.log(`[game] room ${room.code} started`);}
