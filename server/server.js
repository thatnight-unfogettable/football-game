...
function sanitize(room,viewer){const game=room.game;const side=s=>s?engineSideToPlayer(s):null;return {code:room.code,status:room.status,players:Object.fromEntries(Object.entries(room.players).map(([playerSide,p])=>[playerSide,p&&{nickname:p.nickname,ready:p.ready,connected:!!p.ws,side:playerSide}])),you:viewer,choiceOwner:room.choiceOwner,deadline:room.deadline,chat:room.chat.slice(-8),rematch:room.rematch,game:game&&{version:game.version,seed:room.status==='finished'?game.seed:null,settings:game.settings,phase:game.phase,round:game.round,firstBanner:side(game.firstBanner),banTurn:side(game.banTurn),banCount:game.banCount,pickTurn:side(game.pickTurn),candidates:game.phase==='ORDER'?[]:game.candidates.map(player=>player.id),bans:game.bans,picks:{A:game.picks.player.map(player=>player.id),B:game.picks.ai.map(player=>player.id)},layouts:game.layouts,matches:game.matches,winner:side(game.winner),logs:game.logs}};}
