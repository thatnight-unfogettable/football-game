...
import { createGame, startRound, available, banPlayer, pickPlayer, nextRound, simulateSeries } from '../src/game.js';
...
function chooseBest(game,side,action){const ids=available(game);const sorted=[...ids].sort((a,b)=>{const pa=game.players.find(p=>p.id===a),pb=game.players.find(p=>p.id===b);return pb.rating-pa.rating;});return sorted[0];}
...
function createRoom(ws,nickname){const code=roomCode(),side='A',session=token();const room={code,status:'lobby',createdAt:Date.now(),players:{A:{nickname,ready:false,ws,session,continueReady:false,lineupReady:false},B:null},game:null,choiceOwner:null,deadline:null,timer:null,chat:[],rematch:{A:false,B:false},history:[]};rooms.set(code,room);ws.room=code;ws.side=side;send(ws,'SESSION',{code,side,token:session});broadcast(room);console.log(`[room] created ${code}`);}
function joinRoom(ws,code,nickname){const room=rooms.get(code);if(!room)return send(ws,'ERROR',{message:'房间不存在'});if(room.players.B)return send(ws,'ERROR',{message:'房间已满'});if(room.players.A.nickname===nickname)return send(ws,'ERROR',{message:'昵称已被使用'});const session=token();room.players.B={nickname,ready:false,ws,session,continueReady:false,lineupReady:false};ws.room=code;ws.side='B';send(ws,'SESSION',{code,side:'B',token:session});broadcast(room);console.log(`[room] joined ${code}`);}
...
wss.on('connection',(ws,req)=>{
  ...
  ws.on('close',()=>{
    ...
    if(!p)return;
    p.ws=null;
    p.disconnectTimer=setTimeout(()=>finishByForfeit(room,ws.side,'断线超过90秒'),90000);
    broadcast(room);
...
setInterval(()=>{
  ...
  for(const [code,room]of rooms){
    if(room.status==='lobby'&&now-room.createdAt>600000){
      ...
      rooms.delete(code);
    }else if(room.status==='finished'&&now-room.finishedAt>300000){
      clearTimeout(room.timer);
      rooms.delete(code);
    }
  }
  for(const [key,times]of rate)if(!times.some(t=>now-t<10000))rate.delete(key);
},30000);

server.listen(PORT,'0.0.0.0',()=>{
  console.log(`\n⚽ Football BP Server Running`);
